...
const EmpProfilePage = ({ userId }: ProfilePageProps) => {
  ...
  // New function to sync enrollments for all courses
  const syncEnrollments = async (courses: Course[]) => {
    try {
      for (const course of courses) {
        const response = await fetch(`http://localhost:5054/api/Inscription/SyncEnrollments/${course.moodleCourseId}`, {
          method: 'POST',
        });
        if (!response.ok) {
          console.error(`Failed to sync enrollments for course ${course.moodleCourseId}: ${response.statusText}`);
        } else {
          console.log(`Enrollments synced for course ${course.moodleCourseId}`);
        }
      }
    } catch (error) {
      ...
      try {
        ...
        try {
          ...
          if (coursesResponse.ok) {
            ...
            // Sync enrollments for all courses
